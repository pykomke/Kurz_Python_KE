...
class OInfoReply(Reference):
    ...
    ismagic = Bool()
    isalias = Bool()
    namespace = Enum(('builtin', 'magics', 'alias', 'Interactive'))
    type_name = Unicode()
    string_form = Unicode()
    base_class = Unicode()
    length = Integer()
...
class DisplayData(Reference):
    ...
    def _data_changed(self, name, old, new):
        for k,v in new.items():
            ...
            nt.assert_true(isinstance(v, str), "expected string data, got %r" % v)
...
def test_user_variables():
    ...
    nt.assert_equal(user_variables, {'x': {
        'status': 'ok',
        'data': {'text/plain': '1'},
        'metadata': {},
    }})
...
def test_stream():
    ...
    yield nt.assert_equal(content['name'], 'stdout')
    yield nt.assert_equal(content['data'], 'hi\n')
...
def test_display_data():
    ...
    yield nt.assert_equal(data['text/plain'], '1')
