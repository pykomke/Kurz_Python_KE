...
class AsyncHubResult(AsyncResult):
    ...
    def wait(self, timeout=-1):
        ...
        if self._ready:
            return
        local_ids = [msg_id for msg_id in self.msg_ids if msg_id in self._client.outstanding]
        local_ready = self._client.wait(local_ids, timeout)
        if local_ready:
            remote_ids = [msg_id for msg_id in self.msg_ids if msg_id not in self._client.results]
            if not remote_ids:
                self._ready = True
            else:
