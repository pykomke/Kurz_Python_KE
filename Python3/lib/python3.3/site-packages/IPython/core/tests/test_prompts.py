...
class PromptTests(unittest.TestCase):
    ...
    def test_user_ns(self):
        ...
        self.assertEqual(prompt, 'In [bar]')

    def test_builtins(self):
        self.pm.color_scheme = 'NoColor'
        self.pm.in_template = "In [{int}]"
        prompt = self.pm.render('in')
        self.assertEqual(prompt, "In [%r]" % int)

    def test_undefined(self):
        self.pm.color_scheme = 'NoColor'
        self.pm.in_template = "In [{foo_dne}]"
        prompt = self.pm.render('in')
        self.assertEqual(prompt, "In [<ERROR: 'foo_dne' not found>]")
    ...
    def test_render_unicode_cwd(self):
        save = os.getcwd()
        with TemporaryDirectory('ünicødé') as td:
            os.chdir(td)
            self.pm.in_template = r'\w [\#]'
            p = self.pm.render('in', color=False)
            self.assertEqual(p, "%s [%i]" % (os.getcwd(), ip.execution_count))
        os.chdir(save)
